...
namespace SznFramework.SQLite3Helper
{
    public class SQLite3Operate
    {
        ...
        public SQLite3Operate(string InDatabasePath, SQLite3OpenFlags InSQLite3OpenFlags)
        {
            ...
            if (SQLite3Result.OK == SQLite3.Open(InDatabasePath.ConvertToUTF8Bytes(),
        ...
        public bool ValueExists(string InTableName, string InProperty, object InValue)
        {
            ...
            stringBuilder.Append("SELECT * FROM ")
                         ...
                         .Append(InProperty)
                         .Append(" = ")
                         .Append(SQLite3Utility.ConvertToSqlValue(InValue));
        ...
        public bool ValueExists<T>(T InObject, int InPropertyIndex = 0) where T : SyncBase
        {
            ...
            stringBuilder.Append("SELECT * FROM ")
                         ...
                         .Append(" = ")
                         .Append(SQLite3Utility.ConvertToSqlValue(property.Infos[InPropertyIndex].GetValue(InObject, null),
        ...
        public bool ValueExists<T>(object InValue, int InPropertyIndex = 0) where T : SyncBase
        {
            ...
            stringBuilder.Append("SELECT * FROM ")
                         ...
                         .Append(property.Infos[InPropertyIndex].Name)
                         .Append(" = ")
                         .Append(SQLite3Utility.ConvertToSqlValue(InValue, property.Infos[InPropertyIndex].PropertyType));
        ...
        public bool CreateTable<T>(bool InOverWrite = true) where T : SyncBase
        {
            ...
            for (int i = 0; i < length; ++i)
            {
                ...
                if (type == SQLite3Config.INT_TYPE
                    || type == SQLite3Config.BOOL_TYPE
                    || type == SQLite3Config.LONG_TYPE
                    || type == SQLite3Config.SHORT_TYPE)
                {
                    stringBuilder.Append(" INTEGER ");
                }
                else if (type == SQLite3Config.STRING_TYPE
                         || type == SQLite3Config.CHAR_TYPE
                         || type.IsArray)
                {
                    stringBuilder.Append(" TEXT ");
                }
                else if (type == SQLite3Config.FLOAT_TYPE
                    || type == SQLite3Config.DOUBLE_TYPE)
                {
        ...
        public bool Insert(string InTableName, params object[] InData)
        {
            ...
            for (int i = 0; i < length; ++i)
            {
                stringBuilder.Append(SQLite3Utility.ConvertToSqlValue(InData[i])).Append(", ");
            }
            stringBuilder.Remove(stringBuilder.Length - 2, 2);
            stringBuilder.Append(")");
        ...
        public bool InsertT<T>(T InValue) where T : SyncBase
        {
            ...
            for (int i = 0; i < length; i++)
            {
                ...
                stringBuilder.Append(SQLite3Utility.ConvertToSqlValue(info.GetValue(InValue, null), info.PropertyType)).Append(", ");
            }
            stringBuilder.Remove(stringBuilder.Length - 2, 2);
            stringBuilder.Append(")");
                    ...
                    if (result == SQLite3Result.OK)
                    {
                        for (int i = 0; i < count; i++)
                        {
                            for (int j = 0; j < property.InfosLength; j++)
                            {
                                ...
                                {
                                    if (0 != SQLite3.BindInt(stmt, j + 1, ((bool)property.Infos[j].GetValue(InValue[i], null) ? 1 : 0)))
                                    {
                                        Debug.LogError(SQLite3.GetErrmsg(stmt));
                                        SQLite3.Finalize(stmt);
                                        return false;
                                ...
                                else
                                {
                                    if (0 != SQLite3.BindBlob(stmt, j + 1,
                                           Encoding.UTF8.GetBytes(property.Infos[j].GetValue(InValue[i], null)
            ...
            string InConditionFieldName, object InConditionValue)
        {
            return Update(InTableName, InUpdateFieldName, InUpdateValue,
                InConditionFieldName + " = " + SQLite3Utility.ConvertToSqlValue(InConditionValue));
        }
        ...
        public bool Update(string InTableName, string InUpdateFieldName, object InUpdateValue, string InCondition)
        {
            ...
            stringBuilder.Append("UPDATE ").Append(InTableName)
                ...
                .Append(" = ")
                .Append(SQLite3Utility.ConvertToSqlValue(InUpdateValue))
        ...
        public bool UpdateMultiValue(string InTableName, string[] InFieldName, object[] InValues, string InCondition)
        {
            ...
            for (int i = 0; i < length; i++)
            {
                stringBuilder.Append(InFieldName[i])
                    .Append(" = ")
                    .Append(SQLite3Utility.ConvertToSqlValue(InValues[i]))
                    .Append(", ");
            }
            stringBuilder.Remove(stringBuilder.Length - 2, 2);
        ...
        public bool UpdateT<T>(T InValue) where T : SyncBase
        {
            ...
            for (int i = 1; i < length; i++)
            {
                ...
                stringBuilder.Append(property.Infos[i].Name)
                    .Append(" = ")
                    .Append(SQLite3Utility.ConvertToSqlValue(info.GetValue(InValue, null), info.PropertyType))
                    .Append(", ");
            }
            stringBuilder.Remove(stringBuilder.Length - 2, 2);
            stringBuilder.Append(" WHERE ")
                ...
                .Append(" = ")
                .Append(SQLite3Utility.ConvertToSqlValue(property.Infos[0].GetValue(InValue, null), property.Infos[0].PropertyType));
        ...
        public void UpdateOrInsert<T>(T InT) where T : SyncBase
        {
            ...
            if (ValueExists(InT)) UpdateT(InT);
            else InsertT(InT);
        }
        #endregion
                                           ...
                                           string[] InConnectOperators = null)
        {
            ...
            for (int i = 0; i < length; i++)
            {
                stringBuilder.Append(InConditionFieldName[i])
                        ...
                        .Append(InOperator[i])
                        .Append(" ")
                        .Append(SQLite3Utility.ConvertToSqlValue(InConditionValue[i]));
                if (i < connectLength)
                    stringBuilder.Append(" ")
            ...
            string InConditionValue)
        {
            ...
            stringBuilder.Append("SELECT ")
                         ...
                         .Append(" = ")
                         .Append(SQLite3Utility.ConvertToSqlValue(InConditionValue));
        ...
        public T SelectTbyId<T>(string InId) where T : SyncBase, new()
        {
            ...
            stringBuilder.Append("SELECT * FROM ")
                ...
                .Append(property.Infos[0].Name)
                .Append(" = ")
                .Append(SQLite3Utility.ConvertToSqlValue(InId));
        ...
        public T SelectTByKeyValue<T>(int InPropertyIndex, object InExpectedValue) where T : SyncBase, new()
        {
            ...
            stringBuilder.Append("SELECT * FROM ")
                         ...
                         .Append(property.Infos[InPropertyIndex].Name)
                         .Append(" = ")
                         .Append(SQLite3Utility.ConvertToSqlValue(InExpectedValue))
                         .Append("");
